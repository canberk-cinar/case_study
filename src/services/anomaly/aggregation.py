...
quality.py's IQR vs MAD and normalization.py's rank vs min-max):
  ...
  - Equal weights (1/5 each): the most defensible default for a genuinely unsupervised ensemble —
    with no labeled ground truth to justify trusting one heuristic more than another, treating all
    five perspectives as equally credible is the honest baseline.
