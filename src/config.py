...
class Settings(BaseSettings):
    ...
    # Single switch that swaps BOTH RAGContainer Selectors (embedding_provider and llm_provider)
    # at once: "openrouter" (cloud, this machine's RAM-constrained stand-in for local Ollama) or
    # "ollama" (fully local, once installed). Changing this one value in .env is the entire swap;
    # no code change needed (see RAGContainer.DEFAULT_CONFIG, which reads it directly).
    LLM_PROVIDER: str = "openrouter"

    # Feeds Case 8's RAGContainer default config (its `openrouter_*` keys): the LLM provider
    # Case 9's policy_explanation agent ultimately calls through. The brief's original spec for
    # Case 8/9 was local-only (Ollama); OpenRouter's free tier is used instead by default since
    # this machine's limited RAM rules out running a local model well. Real model name and API
    # key belong in .env, never hardcoded here.
